    ...
    fun sqlNodeToPlanner(session: DbSession, stmt: Node): LogicalPlanner {
        when (stmt.javaClass) {
            SelectStatement::class.java -> {
                 ...
                 *                      aggregate(比如count(1), max(age), sum(age)等)
                 *                          |
                 *                      projection
                 *                          |
                 *                        limit
                ...
                var topLevel: LogicalPlanner = projection

                // 如果projection中有聚合函数（而不是普通单行计算函数），需要在projection之上增加聚合算子，并且projection计算的输出遇到聚合函数时要输出聚合函数要求的列
                if (projection.hasAggregateFunc()) {
                    // 这里要求projection中各列要么是分组中的列，要么是聚合函数调用
                    val aggregate = AggregatePlanner(session, projection.columns)
                    aggregate.addChild(projection)
                    topLevel = aggregate
                }
